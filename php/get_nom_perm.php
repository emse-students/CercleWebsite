...
while ($donnees = $req->fetch())
{
    ...
    $rep = $bdd -> prepare("SELECT u.id_user, u.prenom, u.nom, u.login FROM user u, membre_perm mp WHERE mp.id_user=u.id_user and id_nom_perm=?");
	...
	while ($donnees2 = $rep->fetch())
	{
	    ...
	    if ($donnees2["nom"]==""){
			$donnees2["nom"]=explode(".",$donnees2["login"])[1];
			$donnees2["prenom"]=explode(".",$donnees2["login"])[0];
		}
